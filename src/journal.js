...
export function createJournal({ openWord, openDraft }) {
  ...
  try {
    ...
    if (Array.isArray(stored)) entries = stored.filter(item => item && item.type === 'word' && Number.isInteger(item.index) && item.index >= 0 && item.index < 15 && typeof item.title === 'string' && typeof item.text === 'string').slice(0, 15)
